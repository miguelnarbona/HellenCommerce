...
async def lifespan(app: FastAPI):
    global transporte_model, builder
    await log_to_logging_service("INFO", "Iniciando Transporte Service", line_num=45)
    ...
    await log_to_logging_service("INFO", f"TRANSPORTE Service iniciado. Delegará inferencia a {MODEL_UP_URL}", line_num=0)
...
async def process_intent(req: ProcessRequest):
    ...
    try:
        # Aquí se extrae la información y se buscan transportistas
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                model_path = os.getenv("TRANSPORTE_MODEL_PATH", "c:/HellenData/mistral/mistral-7b-instruct-v0.2.Q4_K_M/mistral-7b-instruct-v0.2.Q4_K_M.gguf")
                ...
                if resp.status_code == 200:
                    ...
                    partial_response = "Buscando opciones de transporte disponibles."
        ...
        await log_to_logging_service("INFO", f"Proceso TRANSPORTE completado para {user_id}", line_num=108)
        return {"intent": "TRANSPORTE", "partial": partial_response}
        
    except Exception as e:
        await log_to_logging_service("ERROR", f"Error procesando TRANSPORTE para {user_id}: {e}", line_num=112)
